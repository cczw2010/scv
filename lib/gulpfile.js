...
var imageWatch = scvutil.workSpace+'/'+imagePath+'/**/*.+(jpg|png)';
...
gulp.task('css', function() {
  var cssDst = scvutil.tmpSpace+'/'+cssPath;
  var combined = combiner.obj([
      gulp.src(cssWatch),
...
gulp.task('release', function() {
  var combined = combiner.obj([
      gulp.src(cssWatch),
      // md5(scvutil.tmpSpace+'/'+htmlPath,{extension: '.html'}),
      gulp.dest(scvutil.revSpace+'/'+cssPath)
    ]);
  // 注销了错误报告  使用combiner是为了防止程序异常退出
  // combined.on('error', console.error.bind(console));
  // combined.on('error',function(err){
